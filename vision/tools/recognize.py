...
class FootPadDetect(VisionTool):
    display_name = "脚垫识别"
    ...
    def __init__(self, params=None):
        ...
        self.params.setdefault("minDist", 200)
        self.params.setdefault("param1", 50)
        self.params.setdefault("param2", 30)
        self.params.setdefault("minRadius", 100)
        self.params.setdefault("maxRadius", 300)
        # 中心ROI比例：ROI半径 = 圆半径 / roi_ratio
        self.params.setdefault("roi_ratio", 3)
        # 缺口判断阈值
        self.params.setdefault("variance_threshold", 15)
        self.params.setdefault("edge_density_threshold", 0.05)
    ...
    def process(self, context: PipelineContext) -> ToolResult:
        ...
        minDist = float(self.params.get("minDist", 200))
        param1 = float(self.params.get("param1", 50))
        param2 = float(self.params.get("param2", 30))
        minRadius = int(self.params.get("minRadius", 100))
        maxRadius = int(self.params.get("maxRadius", 300))
        ...
        if circles is None:
            return ToolResult(
                success=True, passed=False,
                processed_image=img,
                data={"has_gap": True, "result": "未检测到螺丝孔"},
                message="未检测到螺丝孔"
            )
        ...
        # 3. 提取中心ROI区域
        roi_ratio = int(self.params.get("roi_ratio", 3))
        roi_radius = r // roi_ratio
        x1 = max(0, x - roi_radius)
        x2 = min(gray.shape[1], x + roi_radius)
        y1 = max(0, y - roi_radius)
        y2 = min(gray.shape[0], y + roi_radius)
        center_roi = gray[y1:y2, x1:x2]

        # 4. 计算两个关键特征
        gray_variance = float(np.var(center_roi))
        edges = cv2.Canny(center_roi, 30, 90)
        edge_density = float(np.sum(edges == 255) / edges.size) if edges.size > 0 else 0.0

        # 5. 阈值判断
        variance_threshold = float(self.params.get("variance_threshold", 15))
        edge_density_threshold = float(self.params.get("edge_density_threshold", 0.05))

        has_gap = gray_variance > variance_threshold and edge_density > edge_density_threshold
        result = "有缺口" if has_gap else "完整"
        # 有缺口 = 没贴脚垫 = NG (passed=False)
        # 完整 = 已贴脚垫 = OK (passed=True)
        passed = not has_gap
        ...
        # 绘制中心ROI
        cv2.circle(display, (x, y), roi_radius, (0, 0, 255), 2)
        cv2.circle(overlay, (x, y), roi_radius, (0, 0, 255), 2)
        # 标注结果
        color = (0, 255, 0) if passed else (0, 0, 255)
        cv2.putText(display, f"Result: {result}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
        cv2.putText(overlay, f"Result: {result}", (10, 30),
                    cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
        cv2.putText(display, f"Variance: {gray_variance:.2f}", (10, 70),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        cv2.putText(display, f"Edge Density: {edge_density:.4f}", (10, 110),
                    cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
        ...
        if input_source.startswith("region:") and self._full_frame_image is not None:
            ...
            if region_name in context.regions:
                rx, ry, rw, rh = context.regions[region_name]
                # 平移圆标注
                cv2.circle(overlay_full, (x + rx, y + ry), r, (0, 255, 0), 2)
                cv2.circle(overlay_full, (x + rx, y + ry), roi_radius, (0, 0, 255), 2)
                cv2.putText(overlay_full, f"Result: {result}", (10, 30),
                           cv2.FONT_HERSHEY_SIMPLEX, 1, color, 2)
                cv2.putText(overlay_full, f"Variance: {gray_variance:.2f}", (10, 70),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
                cv2.putText(overlay_full, f"Edge Density: {edge_density:.4f}", (10, 110),
                           cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 0, 0), 2)
            overlay = overlay_full
        ...
        return ToolResult(
            ...
            message=f"脚垫识别: {result} (方差={gray_variance:.2f}, 边缘密度={edge_density:.4f})"
        )

    def get_param_widgets(self, parent):
        from PyQt5.QtWidgets import (QDoubleSpinBox, QSpinBox, QHBoxLayout,
                                      QWidget, QLabel, QGroupBox, QVBoxLayout)

        widgets = []

        # 霍夫圆检测参数组
        circle_group = QGroupBox("霍夫圆检测参数")
        circle_layout = QVBoxLayout(circle_group)

        dp_spin = QDoubleSpinBox()
        dp_spin.setRange(0.1, 5.0)
        ...
        circle_layout.addWidget(self._make_row("dp:", dp_spin))

        dist_spin = QSpinBox()
        dist_spin.setRange(1, 1000)
        dist_spin.setValue(int(self.params.get("minDist", 200)))
        dist_spin.valueChanged.connect(lambda v: self.params.update({"minDist": v}))
        circle_layout.addWidget(self._make_row("最小距离:", dist_spin))

        p1_spin = QSpinBox()
        p1_spin.setRange(1, 500)
        p1_spin.setValue(int(self.params.get("param1", 50)))
        p1_spin.valueChanged.connect(lambda v: self.params.update({"param1": v}))
        circle_layout.addWidget(self._make_row("Canny阈值:", p1_spin))
        ...
        circle_layout.addWidget(self._make_row("累加器阈值:", p2_spin))

        min_r = QSpinBox()
        min_r.setRange(1, 10000)
        min_r.setValue(int(self.params.get("minRadius", 100)))
        min_r.valueChanged.connect(lambda v: self.params.update({"minRadius": v}))
        circle_layout.addWidget(self._make_row("最小半径:", min_r))

        max_r = QSpinBox()
        max_r.setRange(1, 10000)
        max_r.setValue(int(self.params.get("maxRadius", 300)))
        max_r.valueChanged.connect(lambda v: self.params.update({"maxRadius": v}))
        circle_layout.addWidget(self._make_row("最大半径:", max_r))

        widgets.append(("", circle_group))

        # 判断参数组
        judge_group = QGroupBox("缺口判断参数")
        judge_layout = QVBoxLayout(judge_group)

        roi_ratio = QSpinBox()
        roi_ratio.setRange(1, 10)
        roi_ratio.setValue(int(self.params.get("roi_ratio", 3)))
        roi_ratio.valueChanged.connect(lambda v: self.params.update({"roi_ratio": v}))
        judge_layout.addWidget(self._make_row("ROI比例(半径/):", roi_ratio))

        var_thresh = QDoubleSpinBox()
        var_thresh.setRange(0, 500)
        var_thresh.setSingleStep(0.5)
        var_thresh.setValue(float(self.params.get("variance_threshold", 15)))
        var_thresh.valueChanged.connect(
            lambda v: self.params.update({"variance_threshold": v}))
        judge_layout.addWidget(self._make_row("方差阈值:", var_thresh))

        edge_thresh = QDoubleSpinBox()
        edge_thresh.setRange(0, 1)
        edge_thresh.setSingleStep(0.01)
        edge_thresh.setDecimals(4)
        edge_thresh.setValue(float(self.params.get("edge_density_threshold", 0.05)))
        edge_thresh.valueChanged.connect(
            lambda v: self.params.update({"edge_density_threshold": v}))
        judge_layout.addWidget(self._make_row("边缘密度阈值:", edge_thresh))

        widgets.append(("", judge_group))
